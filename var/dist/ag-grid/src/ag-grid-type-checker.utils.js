...
function isTextFilterModel(filter) {
    ...
    if (isCombinedFilterModel(filter)) {
        ...
        return filter.filterType === ag_grid_enum_1.FilterType.TEXT;
    }
}
function isNumberFilterModel(filter) {
    ...
    if (isCombinedFilterModel(filter)) {
        ...
        return filter.filterType === ag_grid_enum_1.FilterType.NUMBER;
    }
}
function isSetFilterModel(filter) {
    if (!filter)
        return false;
    return filter.filterType === ag_grid_enum_1.FilterType.SET;
}
function isDateFilterModel(filter) {
    ...
    if (isCombinedFilterModel(filter)) {
        ...
        return filter.filterType === ag_grid_enum_1.FilterType.DATE;
    }
}
...
function isFindOperator(filter) {
    ...
    return (casted &&
        ...
        (casted.value !== undefined ||
            ...
            casted.type.toLowerCase() === ag_grid_enum_1.GeneralFilters.ISNULL.toLowerCase()));
}
function isOperator(val) {
    return Object.values(ag_grid_enum_1.Operators).includes(typeof val === 'string' ? val.toUpperCase() : val);
}
