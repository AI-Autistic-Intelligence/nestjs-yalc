import { getDataloaderToken } from "@nestjs-yalc/data-loader/dataloader.helper";
import { isClass } from "@nestjs-yalc/utils/class.helper";
import { GetContext } from "@nestjs-yalc/utils/nest.decorator";
import returnValue from "@nestjs-yalc/utils/returnValue";
import { Query, UseInterceptors, applyDecorators } from "@nestjs/common";
...
import { checkFinalId, generateDecorators, hasExtraArgs, hasFilters, isIDArg } from "./generic-resolver.type";
export function defineFieldResolver(resolverInfoList, resolver) {
    for (const resolverInfo of resolverInfoList) {
        ...
        if (resolverInfo.relation.relationType === 'one-to-many' ||
                ...
                value: async function (parent, findOptions) {
                    ...
                    if (parentRes !== undefined) {
                        if (hasFilters(findOptions))
                            throw new AgGridError('Cannot specify join arguments and resolver arguments at the same time');
                        return [parentRes, -1];
                    }
                    const dataLoader = await this.moduleRef.resolve(getDataloaderToken(relType), this.contextId);
            ...
            ResolveField(returnValue(AgGridGqlType(relType)), {
                nullable: resolverInfo.agField?.gqlOptions?.nullable,
            })(resolver.prototype, resolverInfo.relation.propertyName, descriptor);
            UseInterceptors(new AgGridInterceptor())(resolver.prototype, resolverInfo.relation.propertyName, descriptor);
            ...
            Parent()(resolver.prototype, resolverInfo.relation.propertyName, 0);
            AgGridArgsSingle({ fieldType: relType, entityType: relType })(resolver.prototype, resolverInfo.relation.propertyName, 1);
...
export function defineGetGridResource(queryName, returnType, resolver, methodOptions) {
    ...
    applyDecorators(...generateDecorators(Query, queryName, methodOptions.returnType ??
        returnValue(AgGridGqlType(returnType)), methodOptions))(resolver.prototype, queryName, descriptor);
    UseInterceptors(new AgGridInterceptor())(resolver.prototype, queryName, descriptor);
    const fieldType = methodOptions.returnType?.() ?? returnType;
    const entityType = !isClass(fieldType) && typeof fieldType === 'function'
        ? fieldType()
        : fieldType;
